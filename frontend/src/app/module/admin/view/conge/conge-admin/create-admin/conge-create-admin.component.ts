...
@Component({
  selector: 'app-conge-create-admin',
  templateUrl: './conge-create-admin.component.html'
})
export class CongeCreateAdminComponent extends AbstractCreateController<CongeDto, CongeCriteria, CongeService>  implements OnInit {
   ...
   private _validCongeCode = true;
    ...
    const isPermistted = await this.roleService.isPermitted('TypeConge', 'add');
    if(isPermistted) {
         this.typeConge = new TypeCongeDto();
